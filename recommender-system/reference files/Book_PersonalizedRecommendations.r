...
scaled_ratings = ratings[,c(8:NCOL(ratings)-1)]
...
scaled_ratings = scale(scaled_ratings)
scaled_ratings = cbind(scaled_ratings, ratings[,c('MovieId','UserId','nrat')])
...
#removes userId and MovieId before fit
fit = randomForest(as.factor(nrat)~., data = model_data_train[,-c(19,20)])
#remove userId and MovieId and actual rating
predictions <- predict(fit, model_data_test[,-c(19,20,21)], type="class")
#predictions <-  predict(fit, model_data_test[,-c(19,20,21)], type="prob")[,2]
...
#first recommend movies based on content.
#Take an aggregated weighted matrix per user per time. These will be used as weights
#take recommended movies features (which are binary). Do dot product (sumproduct) for each predictions features * context weights and sum
#sort based on these values to recommend per context
